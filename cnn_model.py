"""
CNN (1D Convolutional Neural Network) Siber Saldırı Tespit Modeli
Convolution ile local pattern'leri hızlı ve etkili şekilde yakalar
"""
...
class CyberAttackCNN:
    """1D CNN tabanlı siber saldırı tespit modeli"""
    ...
    def build_model(self, filters=[64, 128, 64], kernel_size=3, dropout_rate=0.3, learning_rate=0.001):
        """
        1D CNN modelini oluştur
        
        Args:
            filters: Her Conv1D katmanındaki filter sayısı listesi
            kernel_size: Convolution kernel boyutu
            dropout_rate: Dropout oranı
            learning_rate: Öğrenme oranı
        """
        print("\n" + "="*80)
        print("🏗️  1D CNN MODELİ OLUŞTURULUYOR")
        print("="*80)
        
        model = keras.Sequential(name='CyberAttack_CNN')
        ...
        for i, num_filters in enumerate(filters, start=1):
            ...
            model.add(layers.MaxPooling1D(pool_size=2, name=f'maxpool_{i}'))
        ...
        model.add(layers.Dropout(dropout_rate, name='dropout_dense_1'))
        
        model.add(layers.Dense(64, activation='relu', name='dense_2'))
        model.add(layers.Dropout(dropout_rate, name='dropout_dense_2'))
    ...
    def plot_training_history(self, save_path='models/cnn_training_history.png'):
        ...
        fig.suptitle('CNN Model Eğitim Geçmişi', fontsize=16, fontweight='bold')
        
        # Accuracy
        axes[0, 0].plot(self.history.history['accuracy'], label='Train', linewidth=2, color='#9b59b6')
        axes[0, 0].plot(self.history.history['val_accuracy'], label='Validation', linewidth=2, color='#e74c3c')
        ...
        # Loss
        axes[0, 1].plot(self.history.history['loss'], label='Train', linewidth=2, color='#9b59b6')
        axes[0, 1].plot(self.history.history['val_loss'], label='Validation', linewidth=2, color='#e74c3c')
        ...
        # Precision
        axes[1, 0].plot(self.history.history['precision'], label='Train', linewidth=2, color='#9b59b6')
        axes[1, 0].plot(self.history.history['val_precision'], label='Validation', linewidth=2, color='#e74c3c')
        ...
        # AUC
        axes[1, 1].plot(self.history.history['auc'], label='Train', linewidth=2, color='#9b59b6')
        axes[1, 1].plot(self.history.history['val_auc'], label='Validation', linewidth=2, color='#e74c3c')
    ...
    def plot_confusion_matrix(self, cm, save_path='models/cnn_confusion_matrix.png'):
...
def main():
    """Ana fonksiyon - CNN modeli eğitimi"""
    print("\n" + "="*80)
    print("🚀 1D CNN SİBER SALDIRI TESPİT MODELİ")
    print("="*80 + "\n")
    
    # Veriyi yükle
    print("📂 LSTM verisi yükleniyor (CNN de aynı sequence verisini kullanır)...")
    lstm_dir = 'lstm_data'
    ...
    cnn_model = CyberAttackCNN(
        sequence_length=metadata['sequence_length'],
        n_features=metadata['n_features']
    )
    
    cnn_model.build_model(
        filters=[64, 128, 64],  # 3 Conv1D katmanı
        kernel_size=3,
        dropout_rate=0.3,
        learning_rate=0.001
    )
